          ...
          <div className="mt-6 flex items-center justify-end gap-2">
            <Dialog.Close asChild>
              <button
                type="button"
                disabled={loading}
                className="h-11 px-4 rounded-[var(--radius-field)] text-[14px] font-semibold text-[var(--color-text-muted)] hover:text-[var(--color-text-strong)] hover:bg-[var(--color-surface-2)] disabled:opacity-50 transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-accent)]"
              >
                {cancelLabel}
              </button>
            </Dialog.Close>
            <button
              ...
              onClick={onConfirm}
              disabled={loading}
              className={`inline-flex items-center gap-2 h-11 px-5 rounded-[var(--radius-field)] text-[14px] font-semibold text-white ${confirmBg} active:scale-[0.97] disabled:opacity-50 disabled:active:scale-100 transition focus-visible:outline-none focus-visible:ring-2 ${confirmRing}`}
            >
              {loading && <Spinner />}
