...
import { ExternalGameSearchBar, type GameSelection } from './ExternalGameSearchBar'
...
export function GameForm({ initial, mode }: Props) {
  ...
  const [genre, setGenre] = useState(initial?.genre ?? GAME_GENRES[0])
  const [playedDate, setPlayedDate] = useState(
    initial?.playedDate ?? new Date().toISOString().slice(0, 10),
  )
  ...
  const [rawgId, setRawgId] = useState<number | null>(initial?.rawgId ?? null)
  const [coverUrl, setCoverUrl] = useState<string | null>(initial?.coverUrl ?? null)
  const [externalSource, setExternalSource] = useState<'rawg' | null>(
  ...
  const crud = useCrudForm({
    onSubmit: async () => {
      ...
      const url = mode === 'create' ? '/api/games' : `/api/games/${initial?.id}`
    ...
    deleteAction: initial?.id
      ? { url: `/api/games/${initial.id}`, redirectTo: '/games' }
      : undefined,
  })
      ...
      <section className="rounded-[var(--radius-toss)] bg-[var(--color-surface)] p-6 shadow-[var(--shadow-toss)] space-y-5">
        <div>
          ...
          <ExternalGameSearchBar
            ...
            onSelect={(sel: GameSelection) => {
              setTitle(sel.title)
              if (sel.byline) setDeveloper(sel.byline)
              if (sel.genre) setGenre(sel.genre)
              setRawgId(sel.externalId)
              setCoverUrl(sel.coverUrl ?? null)
              setExternalSource('rawg')
            }}
            onClear={() => {
              setRawgId(null)
              setCoverUrl(null)
        ...
        <div>
          <label className={labelCls}>개발사</label>
          <input
            value={developer}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            <label className={labelCls}>플레이한 날짜</label>
            <input
              type="date"
              value={playedDate}
        ...
        <div className="pt-2 border-t border-[var(--color-border)]">
          <Toggle
            ...
            label="모두의 게임관에 공개"
            description="이 게임의 한줄평·별점·제목·개발사를 모두의 게임관에서 다른 사람도 볼 수 있어요"
          />
        </div>
      </section>
