import { Suspense } from 'react'
import { redirect } from 'next/navigation'
import { getCurrentUser } from '@/lib/auth'
import { spKey } from '@/lib/sp-key'
import { MediaListPageBody, MediaResultsSkeleton } from '@/components/media/MediaListResults'
import { BOOKS_PAGE_CONFIG } from '@/components/media/mediaPageConfig'

interface SP {
  searchParams: Promise<{
    genre?: string
    tag?: string
    year?: string
    q?: string
    sort?: string
    page?: string
  }>
}

export default async function BooksPage({ searchParams }: SP) {
  const me = await getCurrentUser()
  if (!me) redirect('/login?next=/books')
  const sp = await searchParams

  return (
    <div className="space-y-6">
      <Suspense fallback={null}>
        <MediaListPageBody.Controls config={BOOKS_PAGE_CONFIG} />
      </Suspense>
      <Suspense fallback={<MediaResultsSkeleton />} key={spKey(sp)}>
        <MediaListPageBody.Results config={BOOKS_PAGE_CONFIG} sp={sp} userId={me.id} />
      </Suspense>
    </div>
  )
}
