...
export const metadata: Metadata = { title: '영화관 통계' }

export default async function MovieStatsPage() {
  const me = await getCurrentUser()
  if (!me) redirect('/login?next=/movies/stats')

  // 전용 페이지 — API fetch 없이 서버에서 직접 집계 (연도는 KST, 홈과 동일 소스)
  const dashboard = await getMovieDashboard(db, me.id, currentKstYear())

  return (
    <div className="space-y-6">
      <div className="flex items-baseline justify-between">
        <h1 className="text-[24px] font-bold text-[var(--color-text-strong)]">📊 영화관 통계</h1>
        <Link
          href="/movies"
      ...
      <StatsDashboard domain="movies" data={dashboard} />
    </div>
  )
}
