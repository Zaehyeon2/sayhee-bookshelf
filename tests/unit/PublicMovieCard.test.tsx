...
describe('PublicMovieCard', () => {
  ...
  it('clickable — navigates to detail page when tmdbId is present', () => {
    const { container } = render(<PublicMovieCard item={baseProps} />)
    const anchor = container.querySelector('a')
    expect(anchor).not.toBeNull()
    expect(anchor!.getAttribute('href')).toBe('/works/movie/157336')
  })

  it('clickable — falls back to /works search when tmdbId is null', () => {
    const { container } = render(<PublicMovieCard item={{ ...baseProps, tmdbId: null }} />)
    const anchor = container.querySelector('a')
